...
const mapApiToUi = (v: ApiVoucher): Voucher => {
    const type: Voucher["type"] = v.discount_type === "percentage" ? "percentage" : "fixed_amount"
    const status: Voucher["status"] = v.active ? "active" : "inactive"
    return {
        id: String(v.id),
        code: v.code,
        name: v.code, // fallback if API doesn't provide
        description: "",
        type,
        value: Number(v.value),
        min_order_amount: v.min_order_value ?? undefined,
        max_discount_amount: undefined,
        usage_limit: v.max_uses,
        used_count: v.current_uses,
        start_date: v.created_at || new Date().toISOString(),
        end_date: v.expiry_date || new Date().toISOString(),
        status,
        applicable_to: "all",
        created_by: "Admin",
...
const mapUiToApi = (v: Partial<Voucher>) => ({
    code: v.code,
    discount_type: v.type === "percentage" ? "percentage" : "fixed",
    value: v.value,
    min_order_value: v.min_order_amount ?? null,
    max_uses: v.usage_limit ?? 1,
    active: v.status ? v.status === "active" : true,
...
export function VoucherManagement() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const list = (res as any)?.data?.data ?? res ?? []
            const ui = (Array.isArray(list) ? list : []).map(mapApiToUi)
            setVouchers(ui)
        }
        load()
    }, [])
    ...
    const toggleVoucherStatus = async (voucherId: string) => {
        const current = vouchers.find((v) => v.id === voucherId)
        if (!current) return
        const nextStatus = current.status === "active" ? "inactive" : "active"
        setVouchers(vouchers.map((v) => (v.id === voucherId ? { ...v, status: nextStatus } : v)))
        try {
            await voucherService.update(String(voucherId), { active: nextStatus === "active" })
        } catch {
            setVouchers(vouchers.map((v) => (v.id === voucherId ? { ...v, status: current.status } : v)))
        }
    }

    const handleCreateVoucher = async (voucherData: Omit<Voucher, "id" | "used_count" | "created_at">) => {
        const payload = mapUiToApi(voucherData)
        const res = await voucherService.create(payload)
        const created = (res as any)?.data ?? res
        const ui = mapApiToUi(created as ApiVoucher)
        // merge UI-only fields
        ui.name = voucherData.name
        ui.description = voucherData.description
        setVouchers([...vouchers, ui])
        toast.success("Tạo voucher thành công")
        setIsCreateDialogOpen(false)
    }

    const handleEditVoucher = async (voucherData: Voucher) => {
        const payload = mapUiToApi(voucherData)
        await voucherService.update(String(voucherData.id), payload)
    ...
    const handleDeleteVoucher = async () => {
        if (deletingVoucher) {
            await voucherService.remove(String(deletingVoucher.id))
    ...
    return (
        <div className="space-y-6">
                        ...
                        <div key={voucher.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => {
                                            // open detail modal; optionally fetch latest
                                    ...
                                    <button
                                        ...
                                        className={`text-sm font-medium ${voucher.status === "active"
            ...
            {/* View details dialog */}
            {isViewDialogOpen && selectedVoucher && (
                <div className="fixed inset-0 bg-black/50 bg-opacity-40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        <div className="flex items-start justify-between mb-4">
                            <h3 className="text-lg font-semibold">Chi tiết voucher</h3>
                            <button className="text-gray-500 hover:text-gray-700" onClick={() => setIsViewDialogOpen(false)}>✕</button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                <div className="text-xs text-gray-500">Mã</div>
                                <div className="font-medium">{selectedVoucher.code}</div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div className="font-medium">{selectedVoucher.type}</div>
                            </div>
                            <div>
                                <div className="text-xs text-gray-500">Giá trị</div>
                                <div className="font-medium">{selectedVoucher.type === "percentage" ? `${selectedVoucher.value}%` : `${selectedVoucher.value.toLocaleString()}đ`}</div>
                            </div>
                            <div>
                                <div className="text-xs text-gray-500">Đơn tối thiểu</div>
                                <div className="font-medium">{selectedVoucher.min_order_amount ? `${selectedVoucher.min_order_amount.toLocaleString()}đ` : "-"}</div>
                            </div>
                            <div>
                                <div className="text-xs text-gray-500">Giới hạn sử dụng</div>
                                <div className="font-medium">{selectedVoucher.used_count}/{selectedVoucher.usage_limit}</div>
                            </div>
                            <div>
                                <div className="text-xs text-gray-500">Ngày bắt đầu</div>
                                <div className="font-medium">{new Date(selectedVoucher.start_date).toLocaleDateString("vi-VN")}</div>
                            </div>
                            <div>
                                <div className="text-xs text-gray-500">Ngày kết thúc</div>
                                <div className="font-medium">{new Date(selectedVoucher.end_date).toLocaleDateString("vi-VN")}</div>
                            </div>
                            <div className="md:col-span-2">
                                <div className="text-xs text-gray-500">Mô tả</div>
                                <div className="font-medium text-gray-700">{selectedVoucher.description || "-"}</div>
                            </div>
                            <div className="md:col-span-2 flex justify-end gap-2 pt-2">
                                <button className="px-3 py-2 rounded-md border" onClick={() => setIsViewDialogOpen(false)}>Đóng</button>
                            </div>
                        </div>
                    </div>
                </div>
