...
import { TableQRGeneratorDialog } from "./table-qr-generator";
...
export function TableManagement({
  ...
  const [selectedTableForReservation, setSelectedTableForReservation] =
    useState<TableAttributes | null>(null);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "cleaning":
        return (
          <Badge className="bg-gray-100 text-gray-800">Đang dọn dẹp</Badge>
        );
      default:
  ...
  const filteredTables = tables.filter((table) => {
    ...
    const matchesTerm =
      ...
      (typeof table.location === "string" &&
        String(table.location).toLowerCase().includes(term)) ||
      (table.location?.area &&
        table.location.area.toLowerCase().includes(term)) ||
      (table.location?.floor !== undefined &&
        String(table.location.floor).includes(term));
    const matchesStatus =
  ...
  const validateForm = (
    ...
    if (!cancel_minutes)
      errors.cancel_minutes = "Thời gian hủy không được để trống";
    else if (Number(cancel_minutes) < 0)
  ...
  const handleDeleteTable = async (id: string) => {
    try {
      ...
      if (response) {
        setTables((prev) =>
          prev.map((table) =>
            table.id === id ? { ...table, deleted_at: new Date() } : table
          )
        );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        <div className="flex flex-col sm:flex-row gap-4 flex-1">
          ...
          <Select
            ...
            onValueChange={setTableStatusFilter}
          >
      ...
      {filteredTables.length === 0 ? (
        <div className="text-center">HIỆN TẠI CHƯA CÓ DỮ LIỆU BÀN</div>
      ) : (
                        ...
                        <>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setAmenities(
                                table.amenities
                                  ? Object.entries(table.amenities).map(
                                      ([label, value]) => ({
                                        label,
                                        value: String(value),
                                      })
      ...
      {/* View Table Dialog */}
      <Dialog
        open={isViewTableDialogOpen}
        onOpenChange={setIsViewTableDialogOpen}
      >
            ...
            <div className="grid grid-cols-2 gap-8 py-4">
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  <Label>Vị trí</Label>
                  <div>
                    {formatLocation(selectedTable.location) || "Không có"}
                  </div>
                </div>
                <div className="grid gap-2">
              ...
              <div className="space-y-4">
                <div className="grid gap-2">
                  ...
                  <div>
                    {Array.isArray(selectedTable.panorama_urls) &&
                    selectedTable.panorama_urls.length > 0 ? (
            ...
            >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      type="number"
                      min="0"
                      defaultValue={selectedTable.deposit}
                      required
                      className="w-full"
                    />
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="edit-cancel_minutes">
                      Thời gian hủy (phút) *
                    </Label>
                    <Input
                      id="edit-cancel_minutes"
                      name="cancel_minutes"
                      type="number"
                      min="0"
                      defaultValue={selectedTable.cancel_minutes}
                      required
                      className="w-full"
                    />
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          id="edit-location_area"
                          name="location_area"
                          defaultValue={selectedTable.location?.area ?? ""}
                        />
                      </div>
                      <div>
                        <Label htmlFor="edit-location_floor">Tầng</Label>
                        <Input
                          id="edit-location_floor"
                          name="location_floor"
                          type="number"
                          min="0"
                          defaultValue={selectedTable.location?.floor ?? ""}
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          min="0"
                          defaultValue={
                            selectedTable.location?.coordinates?.x ?? ""
                          }
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          min="0"
                          defaultValue={
                            selectedTable.location?.coordinates?.y ?? ""
                          }
                        />
                  ...
                  <div className="grid gap-2">
                    <div className="flex justify-between items-center">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          document
                            .getElementById("edit_panorama_upload")
                            ?.click()
                        }
                        className="bg-black text-white hover:bg-gray-800"
                      >
                        <Plus className="h-4 w-4 mr-2" />{" "}
                        {isSubmitting ? "Đang xử lý..." : "Thêm ảnh mới"}
                      </Button>
                      <Input
              ...
              <DialogFooter>
                ...
                <Button
                  type="submit"
                  className="bg-black text-white hover:bg-gray-800"
                  disabled={isSubmitting || Object.keys(editErrors).length > 0}
                >
