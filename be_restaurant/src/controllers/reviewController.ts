...
export const getReviewByDishId = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const review = await reviewService.findByDishId(req.params.dishId);
    res.json({ status: "success", data: review });
  } catch (error) {
    next(error);
  }
  ...
  try {
    ...
    if (payload.type === "dish") {
      if (!payload.order_item_id && !payload.dish_id) {
        throw new AppError(
          "Dish review must reference order_item_id or dish_id",
          400
        );
      }
    }
    if (payload.type === "table") {
      if (!payload.table_id && !payload.order_id) {
        throw new AppError(
          "Table review must reference table_id or order_id",
          400
        );
      }
    }
