...
export const createReservation_app_user = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      const when = reservation.reservation_time ? new Date(reservation.reservation_time).toISOString() : undefined
      const content = `Đặt bàn của bạn đã được tạo${when ? ` — thời gian: ${when}` : ""}. Mã: ${reservation.id}`
      ...
      console.error('[createReservation_app_user] failed to create notification', e)
    }
...
export const getReservationsByTableAndDate_app_user = async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.user?.id) {
      throw new AppError('Unauthorized', 401)
    }
    ...
    const rows = Array.isArray(reservations) ? reservations : (reservations as any).rows || []
    res.json(rows)
  } catch (error) {
    next(error)
  }
