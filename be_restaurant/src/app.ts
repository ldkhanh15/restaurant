...
app.use((req, res, next) => {
  ...
  if (process.env.NODE_ENV !== "production") {
    try {
      ...
      const masked = hasAuth
        ? authHeader.length > 12
          ? `${authHeader.slice(0, 12)}...`
          : authHeader
        : "";
      const queryToken =
        req.query && (req.query.access_token || req.query.token) ? true : false;
      ...
      if (hasAuth || queryToken || hasCookie) {
        logger.info(
          `[dev-auth-debug] authHeader=${hasAuth} masked=${masked} queryToken=${queryToken} cookieHeader=${hasCookie}`
        );
      }
    } catch (e) {
...
if (process.env.NODE_ENV !== "production") {
  app.get("/payment/failed", (req, res) => {
    ...
    if (accept.includes("text/html")) {
      return res.send(
        `<html><body><h1>Payment failed</h1><p>Reason: ${reason}</p></body></html>`
      );
    }
...
if (process.env.NODE_ENV !== "production") {
  app.get("/ReturnUrl", (req, res, next) => {
    try {
      ...
      return paymentAppUserController.vnpayCallback(
        req as any,
        res as any,
        next as any
      );
    ...
    try {
      ...
      try {
        const paymentService =
          require("./services/payment_app_userService").default;
        ...
        return res.json({
          ...
          signData,
          expected,
          verifyResult: "error computing verifyVnpayReturn: " + String(e),
        });
      }
    } catch (err) {
