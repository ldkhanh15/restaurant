...
class EventBookingAppUserService extends BaseService<EventBooking> {
  ...
  async findBookingByIdWithDetails(id: string, userId: string) {
    const booking = await this.findOne({ id, user_id: userId }, { include: includeDetails })
    if (!booking) {
      throw new AppError("Booking not found or you do not have permission to view it", 404)
    }
    return booking
  }
   ...
   */
  async createBooking(data: { userId: string; eventId: string; numberOfTickets: number }) {
    ...
    return sequelize.transaction(async (transaction) => {
      ...
      await event.decrement("capacity", { by: numberOfTickets, transaction })
  ...
  async cancelBooking(bookingId: string, userId: string) {
    return sequelize.transaction(async (transaction) => {
      const booking = await this.findOne({ id: bookingId, user_id: userId }, { transaction, lock: true })
      if (!booking) {
        throw new AppError("Booking not found or you do not have permission to cancel it", 404)
      }
      ...
      await Event.increment("capacity", {
        by: (booking as any).number_of_tickets,
